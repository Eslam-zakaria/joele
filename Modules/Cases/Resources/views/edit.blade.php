                ...
                <div class="card">
                    <div class="card-body">
                        <div class="basic-form">
                            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('admin.cases.update', $case->id) }}">
                                @method('PUT')
                                @csrf
                                ...
                                <div class="form-row" style="margin-bottom:  10px">
                                    <div class="form-group col-md-6 @error('image_before') has-danger @enderror">
                                        ...
                                        <input type="file"
                                               ...
                                               class="dropify"
                                               data-height="200"
                                               data-default-file="{{ $case->image_before }}"/>
                                    ...
                                    <div class="form-group col-md-6 @error('image_after') has-danger @enderror">
                                        ...
                                        <input type="file"
                                               ...
                                               class="dropify"
                                               data-height="200"
                                               data-default-file="{{ $case->image_after }}"/>
                                        ...
                                        <select class="custom-select form-control city-type" id="branches_select_input" name="branch_id">
                                            ...
                                            @foreach($branches as $branch)
                                                <option value="{{ $branch->id }}"
                                                        ...
                                                        {{ $case->branch_id == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div  class="form-group col-md-4 @error('category_id') has-danger @enderror">
                                        ...
                                        <select class="custom-select form-control city-type" id="category_id" name="category_id">
                                            ...
                                            @foreach($categories as $category)
                                                <option value="{{$category->id}}" {{ $case->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                            @endforeach
                                        </select>
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $.each($('#branches_select_input').find(":selected").data('doctors'), function (val, key) {

                if(key == '{{ $case->doctor_id }}'){

                    $('#doctors_select_input').append('<option value="'+ key +'" selected>'+ val +'</option>')
