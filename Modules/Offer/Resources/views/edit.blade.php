...
@section('title', "Edit a Offer ( $offer->name )")
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="basic-form">
                            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('admin.offers.update', $offer->id) }}">
                                @method('PUT')
                                @csrf
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-4 @error('image') has-danger @enderror">
                                        ...
                                        <input type="file"
                                               ...
                                               class="dropify"
                                               data-height="200"
                                               data-default-file="{{ $offer->offer_image }}" />
                                                    ...
                                                    <div class="row">
                                                        <div class="form-group bmd-form-group col-md-12 @error("$locale.name") has-danger @enderror">
                                                            ...
                                                            <input type="text"
                                                                   ...
                                                                   value="{{ $offer->translate($locale)->name }}"
                                                                   class="form-control" />
                                    ...
                                    <div class="form-group col-md-12 @error('category_id') has-danger @enderror">
                                        ...
                                        <select class="custom-select form-control city-type" id="categories_select_input" name="category_id">
                                            ...
                                            @foreach($categories as $category)
                                                <option value="{{ $category->id }}" {{ $offer->category_id == $category->id ? 'selected' : '' }} data-branches="{{ $category->branches }}">
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
        ...
        $(document).ready(function () {
            ...
            if( branches != undefined ){
                $.each(branches, function (index, value){
                    ...
                    if( jQuery.inArray(value.id, {{ json_encode($offer->branches->pluck('id')->toArray()) }}) !== -1 ){
                        $('#branches_select_input').append('<option value="'+ value.id +'" selected>'+ value.name +'</option>');
                    } else {
