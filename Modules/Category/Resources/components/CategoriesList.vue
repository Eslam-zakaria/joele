<template>
    <div class="row">
        <div class="table-filters col-md-12 mb-4">
            <div class="row">
                <div class="col-sm-6 col-md-3 mb-1">
                    <div class="input-group  search-area d-xl-inline-flex  mr-1" >
                        <input type="text" v-on:keyup.enter="fetchCategories()" class="form-control" placeholder="Search here..." v-model="q">
                        <div class="input-group-append">
                            <span class="input-group-text">
                                <a href="javascript:void(0)" v-on:click="fetchCategories()"><i class="flaticon-381-search-2"></i></a>
                            </span>
                        </div>
                    </div>
                </div>

                <div class="col-md-1 col-sm-3 mb-1">
                    <div class="input-group  d-xl-inline-flex">
                        <a class="btn btn-success" :href="route('admin.categories.create')" title="New"><i class="la la-plus"></i></a>
                    </div>
                </div>

                <div class="col-md-2 col-sm-6 mb-1 ml-auto">
                    <div class="input-group search-area d-xl-inline-flex">
                        <div class="dropdown " style="width: 100%">
                            <a href="javascript:void(0)" :class="'btn-'+sort.class" class="btn btn-rounded " style="width: 100%" data-toggle="dropdown" aria-expanded="false">
                                <i class="las la-sort-amount-down-alt scale5"></i>
                                {{  sort.label }}
                                <i class="las la-angle-down "></i>
                            </a>
                            <div class="dropdown-menu dropdown-menu-center">
                                <a v-for="categorySort in sorts" v-on:click="changeSort(categorySort)" class="dropdown-item" href="javascript:void(0);"><span :class="'text-'+categorySort.class">{{ categorySort.label }}</span></a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-responsive-md text-small">
                            <thead>
                                <tr>
                                    <th class="width10">#</th>
                                    <th class="text-center">image</th>
                                    <th class="text-center">Name</th>
                                    <th class="text-center">For</th>
                                    <th class="text-center">Status</th>
                                    <th class="text-center">Created At</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="category in categories.data" :id="'element_row_' + category.id">
                                    <td><strong>{{ category.id }}</strong></td>
                                    <td class="text-center"><a :href="category.category_image" data-toggle="lightbox"><img class="p-4 width130" :src="category.category_image" alt="" /></a></td>
                                    <td class="text-center">{{ category.name }}</td>
                                    <td class="text-center">{{ category.listDisplayIn }}</td>
                                    <td class="text-center">
                                        <span :id="'element_span_' + category.id" v-bind:class="category.statusData.class" class="badge">
                                            {{ category.statusData.label }}
                                        </span>
                                    </td>
                                    <td class="text-center">{{ moment(category.created_at).format('DD MMM YYYY, hh:mm A') }}</td>
                                    <td>
                                        <button type="button"
                                                data-placement="top"
                                                data-toggle="tooltip"
                                                title="change status"
                                                :id="'btn_change_element_status_' + category.id"
                                                :data-url="route('api.category.status', {category: category.id})"
                                                v-on:click="openStatusModal(category.id)"
                                                v-bind:class="category.statusData.btn_class"
                                                class="light sharp btn btn-icon mr-1">
                                            <i v-bind:class="category.statusData.btn_icon"></i>
                                        </button>

                                        <button type="button"
                                                data-placement="top"
                                                data-toggle="tooltip"
                                                title="delete category"
                                                :data-url="route('api.category.destroy', {category: category.id})"
                                                :id="'btn_delete_element_' + category.id"
                                                v-on:click="openDeleteModal(category.id)"
                                                class="light sharp btn btn-danger btn-icon mr-1">
                                            <i class="fa fa-trash-o"></i>
                                        </button>

                                        <a :href="route('admin.categories.edit', {category: category.id})" class="light sharp btn btn-info btn-icon mr-1">
                                            <i class="fa fa-pencil-square-o"></i>
                                        </a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>

                        <delete-modal :name="'category'"></delete-modal>
                        <status-modal :name="'category'"></status-modal>

                        <pagination :limit="8" :data="categories" :show-disabled="true" @pagination-change-page="fetchCategories">
                            <span slot="prev-nav">&lt;</span>
                            <span slot="next-nav">&gt;</span>
                        </pagination>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
export default {
    name: 'categories-list',
    data: function () {
        return {
            'q' : '',
            'sort': { 'id' : 'created_at', 'direction': 'DESC', 'label' : 'Sort', 'class' : ''},
            'categories' : {},
            'page' : 1,
            'sorts' : []
        }
    },
    created() {
        this.fetchCategories();
        this.fetchSorts();
    },
    computed: {
    },
    methods: {
        fetchCategories(page = 1) {
            this.page = page;
            axios
                .get(route('api.categories.index', {
                    page: page,
                    q : this.q,
                    sort : this.sort.id,
                    direction : this.sort.direction,
                }))
                .then(response => {
                    this.categories = response.data;
                });
        },
        fetchSorts() {
            this.sorts = {
                1: { 'id' : 'created_at', 'direction': 'DESC', 'label' : 'Created DESC', 'class' : 'dark'},
                2: { 'id' : 'created_at', 'direction': 'ASC', 'label' : 'Created ASC', 'class' : 'dark'},
            };
        },
        changeSort(sort) {
            this.sort = (sort !== undefined) ? this.sort = sort : '';
            this.fetchCategories();
        },
        openDeleteModal(category_id) {
            $('#delete_btn_submit').attr('data-id', category_id)

            $('#delete_modal').modal('show');
        },
        openStatusModal(category_id) {
            $('#change_status_btn_submit').attr('data-id', category_id)

            $('#change_status_modal').modal('show');
        },
    }
}
</script>
